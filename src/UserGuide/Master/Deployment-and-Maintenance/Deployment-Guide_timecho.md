...
| iotdb\_admin_user       | iotdb username (default:root)                                                                                                                                                                                                                                                                                                                                                  | NO       |
| iotdb\_admin_password   | iotdb password (default:root)                                                                                                                                                                                                                                                                                                                                                       | NO       |
...
| dist-plugin     | Upload plugin (udf, trigger, pipe) to the cluster | clustername,-type U(udf)/T(trigger)/P(pipe) -file /xxxx/trigger.jar,Explanation: After the upload is completed, you need to manually execute the commands to create udf, pipe, and trigger                                                                                                                                            |
| upgrade         | Rolling upgrade                          | clustername                                                                                                                                                                                                                                            |
...
#### Cluster lib package upload (upgrade)
```bash
iotdbctl cluster dist-lib default_cluster
```
* Find the yaml file in the default location according to cluster-name and obtain the configuration information of `confignode_servers` and `datanode_servers`
...
```
* Find the yaml file in the default location based on the cluster name and obtain the configuration information for 'datanode.servers'
* Upload udf/trigger/pipe jar package
  After uploading, you need to manually execute the create udf/trigger/pipe command
...
```
* Find the yaml file at the default location based on the cluster name, and obtain the configuration information for `confignode.servers` and `datanode.servers`
* Upload lib package
* Confignode executes stop, replace lib package, start, then datanode executes stop, replace lib package, start
