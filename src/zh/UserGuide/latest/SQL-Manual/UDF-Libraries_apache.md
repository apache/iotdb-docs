...
+ `Segment`：完整性计算是否考虑序列分段。它的取值为 'true' 或 'false'，默认值为 'false'。当考虑序列分段时，会先将序列按照时间戳分布进行分段，之后按照每段计算完整性。
...
+ `window`：窗口大小，它是一个大于0的整数或者一个有单位的正数。前者代表每一个窗口包含的数据点数目，最后一个窗口的数据点数目可能会不足；后者代表窗口的时间跨度，目前支持五种单位，分别是 'ms'（毫秒）、's'（秒）、'm'（分钟）、'h'（小时）和'd'（天）。缺省情况下，全部输入数据都属于同一个窗口。
+ `Segment`：一致性计算是否考虑序列分段。它的取值为 'true' 或 'false'，默认值为 'false'。当考虑序列分段时，会先将序列按照时间戳分布进行分段，之后按照每段计算一致性。
...
+ `window`：窗口大小，它是一个大于0的整数或者一个有单位的正数。前者代表每一个窗口包含的数据点数目，最后一个窗口的数据点数目可能会不足；后者代表窗口的时间跨度，目前支持五种单位，分别是 'ms'（毫秒）、's'（秒）、'm'（分钟）、'h'（小时）和'd'（天）。缺省情况下，全部输入数据都属于同一个窗口。
+ `Segment`：时效性计算是否考虑序列分段。它的取值为 'true' 或 'false'，默认值为 'false'。当考虑序列分段时，会先将序列按照时间戳分布进行分段，之后按照每段计算时效性。
...
本函数用于时间戳修复。根据给定的标准时间间隔，采用最小化修复代价的方法，通过对数据时间戳的微调，将原本时间戳间隔不稳定的数据修复为严格等间隔的数据。适用于考虑分段和不考虑分段下的时间序列修复。在未给定标准时间间隔的情况下，本函数将使用时间间隔的中位数 (median)、众数 (mode) 或聚类中心 (cluster) 来推算标准时间间隔。
...
+ `Segment`：时间戳修复是否考虑序列分段。它的取值为 'true' 或 'false'，默认值为 'false'。当考虑序列分段时，时间戳序列会修复为由多个等间隔子序列首尾相连的序列。
+ `num_interval`：表示修复序列时考虑的时间间隔数量。仅在`method`取值为'mode'时有效，表示按照时间间隔重数排序后所考虑的时间间隔数量。
...
如果`Segment`取值为'true'，本函数将按照分段修复算法对时间戳进行修复。
