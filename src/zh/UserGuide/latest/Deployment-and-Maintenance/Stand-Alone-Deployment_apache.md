...
1. 安装前请确认系统已参照[系统配置](./Environment-Requirements_apache.md)准备完成。
...
| **配置项**  |               **说明**               |        **默认值**         |                    **推荐值**                    |     备注     |
| :---------: | :----------------------------------: |:----------------------:| :----------------------------------------------: | :----------: |
| MEMORY_SIZE | IoTDB DataNode节点可以使用的内存总量 | 根据系统内存自动计算，默认为系统内存的50% | 可按需填写，填写后系统会根据填写的数值来分配内存 | 修改后保存即可，无需执行；重启服务后生效 |
