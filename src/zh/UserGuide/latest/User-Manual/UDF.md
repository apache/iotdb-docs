...
用于计算时间序列的完整性。将输入序列划分为若干个连续且不重叠的窗口，分别计算每一个窗口的完整性，并输出窗口第一个数据点的时间戳和窗口的完整性。

#### 输入类型
仅支持单个输入序列，类型为 INT32 / INT64 / FLOAT / DOUBLE。
...
#### 输出类型 
输出单个序列，类型为DOUBLE，其中每一个数据点的值的范围都是 [0,1].
...
#### 输入类型
支持多个输入序列，类型为 INT32 / INT64 / FLOAT / DOUBLE。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列中共包含$2N-1$个数据点。
...
#### 输出类型 
直方图分桶的值，其中第 i 个桶（从 1 开始计数）表示的数据范围下界为$min+ (i-1)\cdot\frac{max-min}{count}$，数据范围上界为$min+ i \cdot \frac{max-min}{count}$。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE，序列仅包含一个时间戳为 0、值为积分结果的数据点。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE，序列仅包含一个时间戳为 0、值为时间加权平均结果的数据点。
...
#### 输出类型 
输出单个序列，类型为DOUBLE，序列仅包含一个时间戳为 0、值为绝对中位差的数据点。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE，序列仅包含一个时间戳为 0、值为中位数的数据点。
...
#### 输出类型 
输出单个序列，类型与输入相同，序列仅包含一个时间戳为众数第一次出现的时间戳、值为众数的数据点。
...
#### 输出类型 
输出单个序列，类型与输入序列相同。当`error`=0时，序列仅包含一个时间戳为分位数第一次出现的时间戳、值为分位数的数据点；否则，输出值的时间戳为0。
...
#### 输出类型 
输出单个序列，类型为 INT32，序列仅包含一个时间戳为 0、值为周期的数据点。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。该序列是$Q_{LB} $统计量对应的 p 值，时间标签代表偏移阶数。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。该序列按照重采样频率严格等间隔分布。
...
#### 输出类型 
输出单个序列，类型与输入序列相同。该序列的长度为采样数，序列中的每一个数据点都来自于输入序列。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE，序列仅包含一个时间戳为 0、值为总体偏度的数据点。
...
#### 输出类型 
输出单个序列，类型与输入相同，序列仅包含一个时间戳为 0 、值为极差的数据点。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列仅包含一个时间戳为 0、值为总体标准差的数据点。
...
#### 输入类型
多个输入序列，类型为 INT32 / INT64 / FLOAT / DOUBLE
...
#### 输出类型 
输出单个序列，类型为 BOOLEAN，即该数据点是否为缺失异常。
...
#### 输出类型 
输出单个序列，类型与输入数据中对应列的类型相同，序列为输入列修复后的结果。
...
#### 输入类型
仅支持两个输入序列，类型均为 INT32 / INT64 / FLOAT / DOUBLE

#### 输出类型 
输出单个序列，类型为DOUBLE，它是两个序列卷积的结果。序列的时间戳从0开始，仅用于表示顺序。
...
#### 输出类型 
输出单个序列，类型为DOUBLE。它是将第二个序列从第一个序列中去卷积（第一个序列除以第二个序列）的结果。序列的时间戳从0开始，仅用于表示顺序。
...
#### 输出类型 
输出单个序列，类型为DOUBLE，长度与输入相等。序列的时间戳从0开始，仅用于表示顺序。
...
#### 输出类型 
输出单个序列，类型为DOUBLE，它是滤波后的序列，长度与时间戳均与输入一致。
...
#### 输出类型 
输出单个序列，类型为DOUBLE。该序列是一个等距时间序列，它的值是将两个输入序列依次作为实部和虚部进行逆快速傅里叶变换的结果。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列仅包含一个时间戳为 0、值为总体协方差的数据点。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列仅包含一个时间戳为 0、值为两个时间序列的 DTW 距离值。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列仅包含一个时间戳为 0、值为皮尔森相关系数的数据点。
...
#### 输入类型
仅支持一个输入序列，类型为 INT32 / INT64 / FLOAT / DOUBLE。
...
#### 输出类型 
输出单个序列，类型为 DOUBLE。序列中的每一个数据点对应于一个对称子序列，时间戳为子序列的起始时刻，值为对称度。
...
#### 输入类型
单列时序数据，类型为INT32 / INT64 / FLOAT / DOUBLE
...
#### 输入类型
支持多个输入序列，类型可以是任意的，但要满足严格等间隔的要求。
...
#### 输出类型 
输出单个序列，类型为 INT32。输出序列中的每一个数据点对应一个局部最长连续子序列，时间戳为子序列的起始时刻，值为子序列包含的数据点个数。
...
#### 输出类型 
输出单个序列，类型为 INT32。输出序列中的每一个数据点对应一个指定长度连续子序列，时间戳为子序列的起始时刻，值为子序列包含的数据点个数。
...
#### 输出类型
输出单个序列，类型为 DOUBLE。第一行对应模型的一阶系数，以此类推。
...
#### 输出类型
输出单个序列，类型为INT32，长度为`tb*vb`。序列的时间戳从0开始，仅用于表示顺序。
...
#### 输入类型
仅支持两个输入序列，类型为 INT32 / INT64 / FLOAT / DOUBLE。
...
#### 输出类型
输出单个序列，类型为DOUBLE，长度为`1`。序列的时间戳从0开始，序列仅有一个数据点，其时间戳为0，值为两个时间序列的匹配度。
