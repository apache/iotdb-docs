...
本函数对等数量分桶后，桶内进行随机采样。
```sql
EQUAL_SIZE_BUCKET_RANDOM_SAMPLE(expr1,expr2)
```
**参数**
...
```sql
EQUAL_SIZE_BUCKET_AGG_SAMPLE(expr1,expr2)
```
**参数**
...
```sql
EQUAL_SIZE_BUCKET_OUTLIER_SAMPLE(expr1,expr2)
```
**参数说明**
- `proportion`: 采样比例
...
#### 功能描述
根据输入属性 `value` 和 `type` 输出用户指定的常序列。
#### 输入类型
所有类型
#### 输出类型
输出指定的数据类型
...
```sql
NON_NEGATIVE_DIFFERENCE(expr1,expr2)
```
#### 功能描述
...
```sql
NON_NEGATIVE_DERIVATIVE(expr1,expr2)
```
#### 功能描述
