...
+ **热加载：**  
  + 手动修改配置文件 + ```load configuration```：可在 DataNode 运行时修改，修改后通过 Session 或 Cli 发送 ```load configuration``` 命令（SQL）至 IoTDB 使配置生效。
  + ```set configuration```：对于需要整个集群统一的参数，推荐使用此 SQL 修改。详见 SQL 介绍页面。
