...
`Format` 函数介绍：[Format 函数](../SQL-Manual/Basis-Function_apache.md#_7-2-format-函数)

`Date_bin` 函数介绍：[Date_bin 函数](../SQL-Manual/Basis-Function_apache.md#_4-2-date-bin-interval-timestamp-timestamp-timestamp)
...
1. 显式地将一个值转换为指定类型。
2. 可以用于将字符串（varchar）转换为数值类型，或数值转换为字符串类型，V2.0.8-beta 版本起支持 OBJECT 类型强转成 STRING 类型。
3. 如果转换失败，将抛出运行时错误。
