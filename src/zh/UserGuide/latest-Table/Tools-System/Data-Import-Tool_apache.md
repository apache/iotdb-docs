...
| -db          | --database             | 数据将要导入的目标库，只在 `-sql_dialect` 为 table 类型下生效。                                                                                   |-sql_dialect 为 table 时必填；<br>V2.0.10 版本起，当文件格式为 SQL 时，该参数为可选参数，若参数或 SQL 中均未显式指定目标数据库时会进行提示。 | -                                |
...
| -sd      | --success\_dir         | 当`--on_success`为 mv 或 cp 时，mv 或 cp 的目标文件夹。文件的文件名变为文件夹打平后拼接原有文件名                                                                         | 当`--on_success`为mv或cp时需要填写                | `${EXEC_DIR}/success`|
