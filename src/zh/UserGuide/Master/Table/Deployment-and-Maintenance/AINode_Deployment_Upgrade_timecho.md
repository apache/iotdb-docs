...
以下为基于单张 NVIDIA 4090（24 GB 显存）运行模型推理任务的资源配置基准线。AINode 中的模型推理任务支持通过横向扩展显卡数量来提升整体吞吐，通常建议按 1、2、4、8 张显卡四种规格配置机器。
...
| GPU 数量（NVIDIA 4090, 24 GB 显存） | 推荐 CPU 核数 | 推荐内存（GB） | 可支持单变量推理吞吐（QPS） | 可支持协变量推理吞吐（QPS） |
| ------------------------------------- | --------------- | ---------------- | ----------------------------- | ----------------------------- |
| 1 卡                                | 16 核         | 24 GB          | 100                         | 10                          |
| 2 卡                                | 32 核         | 48 GB          | 200                         | 20                          |
| 4 卡                                | 64 核         | 96 GB          | 400                         | 40                          |
| 8 卡                                | 128 核        | 192 GB         | 800                         | 80                          |
...
3. 更新内置模型权重（可选）
   ...
   * 若新版本涉及内置模型更新，相关信息将在[发布历史](../IoTDB-Introduction/Release-history\_timecho.md)中同步。可联系天谋工作人员获取最新权重包，并将权重包替换至 `data/ainode/models/builtin` 目录
