...
2. 确保 AINode 启动成功 
3. 打开 IoTDB cli 控制台
4. 使用 show cluster 语句检查 AINode 节点确保为 Running
   
![](/img/ainode-timer-1.png)

5. 推荐模型文件放在 AINode 安装包相同文件夹下，可新建模型文件夹存放模型文件
6. 注册模型语句：create model <model_name> using uri <uri>
   例子：create model Timer using uri <uri>

![](/img/ainode-timer-2.png)

7. 使用 show models 语句检查模型是否注册成功
